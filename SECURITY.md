...
QuadTecho はリリースタグでバージョン管理しています。最新のリリースのみを
サポート対象としています。

| 対象 | サポート状況 |
| --- | --- |
| 最新リリース（現在 `0.01-beta`） | :white_check_mark: サポート対象 |
| 過去のリリース・タグ | :x: 対象外 |
...
- 対象: 本リポジトリのコード（Python / フロントエンド JavaScript / 設定ファイル）
- 対象外: 依存する外部サービス自体の問題（Google Fonts / Vue.js CDN など。各プロバイダの報告窓口をご利用ください）

## セルフホスト運用者へ

QuadTecho は自己のサーバー上で運用することを想定しています。公開サーバーとして
